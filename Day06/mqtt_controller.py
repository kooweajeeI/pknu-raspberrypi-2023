...
class publisher(Thread):
    def __init__(self):
        ...
        self.host = '210.119.12.65' # 본인 pc ip
        self.port = 1883    # 회사에서는 이 포트 안씀
        self.clientId = 'IOT65'
        self.count = 0
        print('pubisher 스레드 시작')
        self.client = mqtt.Client(client_id=self.clientId) # 설계대로

    def run(self):
        self.client.connect(self.host, self.port)
        # self.client.username_pw_set() # id/pwd 로그인할 때 필요
...
class subscriber(Thread):
    def __init__(self): # 생성자
        ...
        self.host = '210.119.12.65' # Broker IP
        self.port = 1883
        self.clientId = 'IOT65_SUB'
        self.topic = 'pknu/monitor/control/'
        print('subscriber 스레드시작')
